...
function LoadingText() {
  return <div className="text-xs text-muted-foreground py-6 text-center">Загрузка...</div>;
}
...
export default function Dashboard() {
  ...
  // ── Derived metrics from weekly-stats ──────────────────────────────────────
  ...
  // CPL trend — derived from weekly-stats (cpl field + synthetic ltv)
  const cplTrend = weeklyData.map((w) => ({
    week: w.week,
    cpl: w.cpl,
    ltv: Math.round(w.cpl * 7.2), // synthetic LTV ratio kept for chart shape
  }));
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-screen-2xl mx-auto px-4 sm:px-6 py-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-3 mb-4">
          {weeklyLoading ? (
            <div className="col-span-full"><LoadingText /></div>
          ) : (
            <>
              {[
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            ...
            {weeklyLoading ? (
              ...
              <div className="h-44">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={weeklyData} margin={{ top: 4, right: 0, left: -20, bottom: 0 }} barGap={4}>
                    <defs>
          ...
          <DashCard title="CPL vs LTV по неделям" subtitle="Стоимость привлечения и ценность клиента">
            {weeklyLoading ? (
              ...
              <div className="h-40">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={cplTrend} margin={{ top: 4, right: 0, left: -20, bottom: 0 }}>
                    <defs>
                    ...
                    <div key={k.name} className="flex flex-col gap-1">
                      <div className="flex items-center justify-between text-xs">
                        ...
                        <div className="flex items-center gap-2">
                          <span className="text-muted-foreground">{(k.plan || 0).toLocaleString("ru-RU")} {k.unit}</span>
                          <span className="font-mono font-semibold" style={{ color }}>{(k.fact || 0).toLocaleString("ru-RU")}</span>
            ...
            {weeklyLoading ? (
              ...
              <>
                <div className="flex gap-5 flex-wrap">
                  ...
                  <StatPill label="Лучшая неделя" value={`${bestRomi}%`} trend={bestRomiWeek} up icon="Star" />
                </div>
                <div className="h-32">
                  <ResponsiveContainer width="100%" height="100%">
                ...
                <table className="w-full text-xs">
                  ...
                  <tbody>
                    {influencerData.map((inf) => (
                      <tr key={inf.id ?? inf.name} className="border-b border-border/30 last:border-0 hover:bg-muted/30 transition-colors">
                        <td className="py-2 font-medium text-foreground">{inf.name}</td>
                        <td className="py-2 text-right font-mono text-foreground">{(inf.reach || 0).toLocaleString("ru-RU")}</td>
                        <td className="py-2 text-right font-mono text-foreground">{(inf.cpm || 0).toLocaleString("ru-RU")}</td>
                  ...
                  <tfoot>
                    <tr className="border-t border-border/60">
                      ...
                      <td className="py-1.5 text-right font-mono font-bold text-foreground">
                        {influencerData.reduce((s, r) => s + (r.reach || 0), 0).toLocaleString("ru-RU")}
                      </td>
